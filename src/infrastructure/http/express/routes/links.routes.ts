...
const revokeLinkUseCase = new RevokeLinkUseCase(
  enlaceRepository,
  archivoRepository,
  carpetaRepository
);

function esAdminJwt(req: { user?: { rol?: string } }): boolean {
  return String(req.user?.rol ?? '').toUpperCase() === 'ADMIN';
}
...
router.post('/', async (req, res, next) => {
  try {
    ...
    const enlace = await createLinkUseCase.execute(
      ...
      userId,
      { isAdmin: esAdminJwt(req as { user?: { rol?: string } }) }
    );
